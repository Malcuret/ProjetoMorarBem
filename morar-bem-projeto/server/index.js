...
app.post('/api/leads', async (req, res) => { try { const codigo = String(req.body.codigo_imovel || '').trim(); const telefone = String(req.body.telefone || '').replace(/\D/g, ''); if (!codigo || !telefone) return res.status(400).json({ error: 'Codigo do imovel e telefone sao obrigatorios.' }); if (telefone.length < 10 || telefone.length > 15) return res.status(400).json({ error: 'Numero de telefone invalido.' }); const lead = await Lead.create({ codigo_imovel: codigo, telefone }); return res.status(201).json({ message: 'Interesse enviado com sucesso!', lead: { id: lead.id, codigo_imovel: lead.codigo_imovel, telefone: lead.telefone } }); } catch (error) { console.error(error); return res.status(500).json({ error: 'Nao foi possivel salvar o interesse.' }); } });
